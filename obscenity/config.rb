module Obscenity
  class Config
    ...
    DEFAULT_WHITELIST = []
    DEFAULT_BLACKLIST = File.dirname(__FILE__) + "/../blacklist.yml"
    DEFAULT_MADLIBS = File.dirname(__FILE__) + "/../replacement.yml"
    ...
    def madlibs
      @madlibs ||= DEFAULT_MADLIBS
    end

    def madlibs=(value)
      @madlibs = value == :default ? DEFAULT_MADLIBS : value
    end
    
    private
    def validate_config_options
      [@blacklist, @whitelist, @madlibs].each{ |content| validate_list_content(content) if content }
    end
